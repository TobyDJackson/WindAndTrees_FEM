function [ CWS_step ] = define_CURTAILED_CWS_Step( cylinder_data,leaf,drag_coefficient,set, ADD_ROOTS )
...
%'parameterize_leaves.m'
    ...
    MCW_header=sprintf(['** STEP: CWS_Wind\n'... 
    '*Step, name=CWS_Wind, NLGEOM=YES, UNSYMM=YES, INC=50000\n'...
    '**DRAG LOADS\n'...
    '*DYNAMIC\n'...
    '0.1,280, 1e-11, 0.25']);%\n'...
    %'*DLOAD\n'...  %THIS ADDS GRAVITY
    ...
    for index=2:num_cyls-1
       MCW_load=sprintf(['set' num2str(index) ', WDD, 1,' num2str(2*radius(index)*leaf(index)) ',' num2str(drag_coefficient) ',1,CWS_Amp']);
    ...
    if ADD_ROOTS==1
            MCW_dloads='*DLOAD';
            for index=2:num_cyls-1
               MCW_load=sprintf(['Assembly1.Instance1.set' num2str(index) ', WDD, 1,' num2str(2*radius(index)*leaf(index)) ',' num2str(drag_coefficient) ',1,CWS_Amp']);
               %This is WDD, magnitude (varied with AMPLITUDE), effective_D, Drag coefficient, cosine of directions etc (default =1),Xamplitude,Yamplitude
               MCW_dloads=strvcat(MCW_dloads,MCW_load);
            end
     ...
     CWS_step=strvcat(MCW_header,MCW_dloads,output_requests);
