function [ part output_set] = import_model_geometry_with_variable_E_2017( cylinder_data,beam_type,output_set_beams, dashpot_coeff ,FIX_DBH,DBH)
...
%strains along the beam - the default is B31.
    ...
    for index=2:num_cyls-1; %Loop over number of cylinders
        ...
        %There are now 5 levels of material properties - from big to small
        % E starts at literature value and decreases by 10% each step
        if radius(index)/radius(1)>0.8
            beam=sprintf(['*Beam Section, elset=set' num2str(index) ',material=wood1,section=circ\n'...
                num2str(radius(index)) '\n'...
                  '' num2str(new_qsm(index,9)) ',' num2str(new_qsm(index,7)) ',' num2str(new_qsm(index,8)) '']);
        end
        if radius(index)/radius(1)>0.6 & radius(index)/radius(1)<=0.8;
            beam=sprintf(['*Beam Section, elset=set' num2str(index) ',material=wood2,section=circ\n'...
                num2str(radius(index)) '\n'...
                  '' num2str(new_qsm(index,9)) ',' num2str(new_qsm(index,7)) ',' num2str(new_qsm(index,8)) '']);
        end
        if radius(index)/radius(1)>0.4 & radius(index)/radius(1)<=0.6;
            beam=sprintf(['*Beam Section, elset=set' num2str(index) ',material=wood3,section=circ\n'...
                num2str(radius(index)) '\n'...
                  '' num2str(new_qsm(index,9)) ',' num2str(new_qsm(index,7)) ',' num2str(new_qsm(index,8)) '']);
        end
        if radius(index)/radius(1)>0.2 & radius(index)/radius(1)<=0.4;
            beam=sprintf(['*Beam Section, elset=set' num2str(index) ',material=wood4,section=circ\n'...
                num2str(radius(index)) '\n'...
                  '' num2str(new_qsm(index,9)) ',' num2str(new_qsm(index,7)) ',' num2str(new_qsm(index,8)) '']);
        end
        if radius(index)/radius(1)<=0.2;
            beam=sprintf(['*Beam Section, elset=set' num2str(index) ',material=wood5,section=circ\n'...
                num2str(radius(index)) '\n'...
                  '' num2str(new_qsm(index,9)) ',' num2str(new_qsm(index,7)) ',' num2str(new_qsm(index,8)) '']);
        end
